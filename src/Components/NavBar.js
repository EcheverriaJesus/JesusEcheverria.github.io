...
const Navbar = () => {
  ...
  /* Funcion para verfiifcar el Efecto Dark */
  useEffect(() => {
    if (theme === "dark") {
        ...
        <section className={`${abierto ? 'block' : 'hidden'} pt-3 lg:flex lg:items-center lg:w-auto`}>
          <ul className='space-y-3 lg:flex lg:space-x-5 lg:space-y-0'>
            {
              Links.map((link, index) => (
                <li key={index}>
                  <Link to={link.section} spy={true} smooth={true} offset={-80} duration={500} className='text-gray-700 hover:font-semibold duration-500 dark:text-gray-300 dark:hover:text-yellow-300' href={link.section}>{link.name}</Link>  
                </li>
              ))
            }
          </ul>
          ...
          <div className='flex pt-4 gap-2 lg:ml-4 lg:pt-0 lg:pl-4'>
            ...
            <li className='list-none hover:scale-150 duration-200'>
              <a className='pt-5' href="../Documents/CV_JN.pdf" target="_blank" rel="noopener noreferrer">
